const express = require('express');
const router = express.Router();
const { query } = require('../config/database');

// Función helper para validar números de Brinco
function validateBrincoNumbers(numbers, min = 0, max = 39, count = 6) {
    if (!Array.isArray(numbers)) {
        throw new Error('Los números deben ser un array');
    }
    
    if (numbers.length !== count) {
        throw new Error(`Se requieren exactamente ${count} números`);
    }
    
    for (const num of numbers) {
        if (!Number.isInteger(num) || num < min || num > max) {
            throw new Error(`Número inválido: ${num}. Debe estar entre ${min} y ${max}`);
        }
    }
    
    // Verificar números únicos
    const uniqueNumbers = [...new Set(numbers)];
    if (uniqueNumbers.length !== numbers.length) {
        throw new Error('No se permiten números repetidos');
    }
}

// GET /api/brinco - Obtener todos los sorteos
router.get('/', async (req, res) => {
    try {
        const sorteos = await query(`
            SELECT s.id, s.fecha, s.created_at, s.updated_at
            FROM brinco_sorteos s
            ORDER BY s.fecha DESC, s.id DESC
        `);

        // Obtener números para cada sorteo
        for (const sorteo of sorteos) {
            const numeros = await query(`
                SELECT tipo, numero
                FROM brinco_numeros
                WHERE sorteo_id = ?
                ORDER BY tipo, numero
            `, [sorteo.id]);

            // Organizar números por tipo
            sorteo.brincoTradicional = [];
            sorteo.brincoJunior = [];

            for (const { tipo, numero } of numeros) {
                switch (tipo) {
                    case 'tradicional':
                        sorteo.brincoTradicional.push(numero);
                        break;
                    case 'junior':
                        sorteo.brincoJunior.push(numero);
                        break;
                }
            }

            // Ordenar arrays
            sorteo.brincoTradicional.sort((a, b) => a - b);
            sorteo.brincoJunior.sort((a, b) => a - b);
        }

        res.json(sorteos);
    } catch (error) {
        console.error('Error obteniendo sorteos Brinco:', error);
        res.status(500).json({ error: 'Error obteniendo sorteos' });
    }
});

// GET /api/brinco/:id - Obtener un sorteo específico
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        
        const sorteos = await query(`
            SELECT s.id, s.fecha, s.created_at, s.updated_at
            FROM brinco_sorteos s
            WHERE s.id = ?
        `, [id]);

        if (sorteos.length === 0) {
            return res.status(404).json({ error: 'Sorteo no encontrado' });
        }

        const sorteo = sorteos[0];
        
        // Obtener números
        const numeros = await query(`
            SELECT tipo, numero
            FROM brinco_numeros
            WHERE sorteo_id = ?
            ORDER BY tipo, numero
        `, [id]);

        // Organizar números por tipo
        sorteo.brincoTradicional = [];
        sorteo.brincoJunior = [];

        for (const { tipo, numero } of numeros) {
            switch (tipo) {
                case 'tradicional':
                    sorteo.brincoTradicional.push(numero);
                    break;
                case 'junior':
                    sorteo.brincoJunior.push(numero);
                    break;
            }
        }

        res.json(sorteo);
    } catch (error) {
        console.error('Error obteniendo sorteo Brinco:', error);
        res.status(500).json({ error: 'Error obteniendo sorteo' });
    }
});

// POST /api/brinco - Crear nuevo sorteo
router.post('/', async (req, res) => {
    try {
        const { 
            concursoId, 
            fecha, 
            brincoTradicional, 
            brincoJunior
        } = req.body;

        // Validaciones
        if (!concursoId || !fecha) {
            return res.status(400).json({ error: 'ID de concurso y fecha son requeridos' });
        }

        if (!Number.isInteger(concursoId) || concursoId <= 0) {
            return res.status(400).json({ error: 'ID de concurso debe ser un número entero positivo' });
        }

        // Validar formato de fecha
        if (!/^\d{4}-\d{2}-\d{2}$/.test(fecha)) {
            return res.status(400).json({ error: 'Fecha debe estar en formato YYYY-MM-DD' });
        }

        // Validar números (Brinco usa rango 0-39)
        validateBrincoNumbers(brincoTradicional, 0, 39, 6);
        validateBrincoNumbers(brincoJunior, 0, 39, 6);

        // Verificar si ya existe el sorteo
        const existingSorteo = await query('SELECT id FROM brinco_sorteos WHERE id = ?', [concursoId]);
        if (existingSorteo.length > 0) {
            return res.status(409).json({ error: `Ya existe un sorteo con ID ${concursoId}` });
        }

        // Insertar sorteo
        await query('INSERT INTO brinco_sorteos (id, fecha) VALUES (?, ?)', [concursoId, fecha]);

        // Insertar números
        const numerosData = [];
        
        // Brinco Tradicional
        for (const numero of brincoTradicional) {
            numerosData.push([concursoId, 'tradicional', numero]);
        }
        
        // Brinco Junior
        for (const numero of brincoJunior) {
            numerosData.push([concursoId, 'junior', numero]);
        }

        // Insertar todos los números
        if (numerosData.length > 0) {
            const placeholders = numerosData.map(() => '(?, ?, ?)').join(', ');
            const flatData = numerosData.flat();
            await query(`INSERT INTO brinco_numeros (sorteo_id, tipo, numero) VALUES ${placeholders}`, flatData);
        }

        res.status(201).json({ 
            message: 'Sorteo creado exitosamente',
            sorteo: { 
                id: concursoId, 
                fecha,
                brincoTradicional,
                brincoJunior
            }
        });

    } catch (error) {
        console.error('Error creando sorteo Brinco:', error);
        
        if (error.code === 'ER_DUP_ENTRY') {
            return res.status(409).json({ error: 'Ya existe un sorteo con este ID' });
        }
        
        res.status(500).json({ error: error.message || 'Error interno del servidor' });
    }
});

// GET /api/brinco/frequencies - Obtener frecuencias de números
router.get('/frequencies', async (req, res) => {
    try {
        const frequencies = await query(`
            SELECT numero, COUNT(*) as cantidad
            FROM brinco_numeros
            GROUP BY numero
            ORDER BY cantidad DESC, numero ASC
        `);

        // Crear array con todos los números (0-39) y sus frecuencias
        const result = Array.from({ length: 40 }, (_, i) => {
            const found = frequencies.find(f => f.numero === i);
            return {
                numero: i,
                cantidad: found ? found.cantidad : 0
            };
        });

        // Ordenar por cantidad (descendente) y luego por número (ascendente)
        result.sort((a, b) => b.cantidad - a.cantidad || a.numero - b.numero);

        res.json(result);
    } catch (error) {
        console.error('Error obteniendo frecuencias Brinco:', error);
        res.status(500).json({ error: 'Error obteniendo frecuencias' });
    }
});

// DELETE /api/brinco/:id - Eliminar sorteo
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        
        const result = await query('DELETE FROM brinco_sorteos WHERE id = ?', [id]);
        
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Sorteo no encontrado' });
        }

        res.json({ message: 'Sorteo eliminado exitosamente' });
    } catch (error) {
        console.error('Error eliminando sorteo Brinco:', error);
        res.status(500).json({ error: 'Error eliminando sorteo' });
    }
});

module.exports = router;